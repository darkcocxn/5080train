...
class Config:
    ...
    MODEL_ROOT_DIR = PROJECT_ROOT / "output" / "2dcnnv4"
    ...
    VAL_CORE_MAX = 0.010
    VAL_FOCUS_RMSE_WEIGHT = 0.05
    VAL_FOCUS_TAIL_MAE_WEIGHT = 0.0
    VAL_FOCUS_EXTREME_TAIL_MAE_WEIGHT = 0.0
    VAL_FOCUS_TAIL_UNDER_WEIGHT = 0.0
    VAL_FOCUS_EXTREME_TAIL_UNDER_WEIGHT = 0.0
    VAL_MID_TAIL_LOW = 0.005
    VAL_MID_TAIL_HIGH = 0.010
    SELECTION_METRIC = "core_mae_with_small_global_blend"
    SELECTION_CORE_MAE_WEIGHT = 1.0
    SELECTION_GLOBAL_MAE_WEIGHT = 0.15
    SELECTION_RMSE_WEIGHT = 0.05
    SELECTION_FOCUS_WEIGHT = 0.0
    SELECTION_MID_TAIL_WEIGHT = 0.0
    USE_EMA = True
    EMA_DECAY = 0.998
...
def calculate_validation_focus_metrics(y_true_raw: np.ndarray, y_pred_raw: np.ndarray) -> dict[str, float]:
    ...
    core_mask = y_true_raw < Config.VAL_CORE_MAX
    ...
    if np.any(core_mask):
        ...
        core_bias = float(np.mean(core_errors))
        core_under_mae = float(np.mean(under_errors[core_mask]))
    else:
        core_mae = float(np.mean(np.abs(errors)))
        core_bias = float(np.mean(errors))
        core_under_mae = float(np.mean(under_errors))
    ...
    return {
        ...
        "core_mae": float(core_mae),
        "core_bias": float(core_bias),
        "core_under_mae": float(core_under_mae),
...
def calculate_selection_score(metrics: dict[str, float], focus_metrics: dict[str, float]) -> float:
    return float(
        Config.SELECTION_CORE_MAE_WEIGHT * focus_metrics["core_mae"]
        + Config.SELECTION_GLOBAL_MAE_WEIGHT * metrics["mae"]
...
def train():
    ...
    metadata.update(
        {
            ...
            "architecture_revision": "core_distribution_focused_v4",
            "best_weights_name": "best_2dcnn_model.pth",
            ...
            "validation_selection": {
                ...
                "focus_component": "core_focus_score",
                "core_component": "core_mae",
                "core_max": Config.VAL_CORE_MAX,
                "core_mae_weight": Config.SELECTION_CORE_MAE_WEIGHT,
                "global_mae_weight": Config.SELECTION_GLOBAL_MAE_WEIGHT,
                "global_rmse_weight": Config.SELECTION_RMSE_WEIGHT,
    ...
    best_val_core_mae_raw = float("inf")
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        history["val_core_mae_raw"].append(focus_metrics["core_mae"])
        history["val_core_bias_raw"].append(focus_metrics["core_bias"])
        history["val_core_under_mae_raw"].append(focus_metrics["core_under_mae"])
        history["val_core_count"].append(focus_metrics["core_count"])
        ...
        if improved:
            ...
            best_val_core_mae_raw = focus_metrics["core_mae"]
            ...
            tqdm.write(
                ...
                f"Core MAE: {focus_metrics['core_mae']:.6f}, "
    ...
    metadata.update(
        {
            ...
            "best_val_core_mae_raw": float(best_val_core_mae_raw),
    ...
    print(
        ...
        f"Core MAE: {best_val_core_mae_raw:.6f}, "
