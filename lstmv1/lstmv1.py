# -*- coding: utf-8 -*-
"""
LSTM sequence surrogate training script.

Inputs:
- raw ground-motion sequence from txt_path
- scalar structure / wave / damper-layout features
...
class Config:
    ...
    MODEL_TAG = "lstmv1"
    ENV_PREFIX = "LSTM"
    MODEL_FAMILY = "wave_sequence_lstm_scalar_fusion"
    ARCHITECTURE_REVISION = "lstmv1_convstem_bilstm_attention_tailaware"
    ...
    BEST_WEIGHTS_NAME = "best_lstm_model.pth"
    MAE_WEIGHTS_NAME = "best_lstm_mae_model.pth"
    FOCUS_WEIGHTS_NAME = "best_lstm_focus_model.pth"
    EXTREME_UNDER_WEIGHTS_NAME = "best_lstm_extreme_under_model.pth"
    FINAL_WEIGHTS_NAME = "lstm_model.pth"
    EMA_WEIGHTS_NAME = "ema_lstm_model.pth"
    ...
    SEQ_STEM_CHANNELS = [32, 64]
    SEQ_STEM_DROPOUT = 0.08
    LSTM_HIDDEN_DIM = 128
    LSTM_NUM_LAYERS = 2
    LSTM_BIDIRECTIONAL = True
    LSTM_DROPOUT = 0.20
    LSTM_ATTENTION_DIM = 128
    SEQUENCE_PROJECTOR_DIM = 256
...
class SequenceConvStem(nn.Module):
    def __init__(self, in_channels: int):
        super().__init__()
        c0, c1 = Config.SEQ_STEM_CHANNELS
        self.net = nn.Sequential(
            nn.Conv1d(in_channels, c0, kernel_size=7, stride=2, padding=3, bias=False),
            build_conv1d_norm(Config, c0),
            nn.SiLU(inplace=True),
            nn.Dropout(Config.SEQ_STEM_DROPOUT),
            nn.Conv1d(c0, c1, kernel_size=5, stride=2, padding=2, bias=False),
            build_conv1d_norm(Config, c1),
            nn.SiLU(inplace=True),
            nn.Dropout(Config.SEQ_STEM_DROPOUT),
        )
        self.out_dim = c1
...
class AttentionPooling(nn.Module):
    def __init__(self, input_dim: int):
        ...
        self.score = nn.Sequential(
            nn.Linear(input_dim, Config.LSTM_ATTENTION_DIM),
            nn.Tanh(),
            nn.Linear(Config.LSTM_ATTENTION_DIM, 1),
        )
...
class LSTMSequenceSurrogate(nn.Module):
    def __init__(self, sequence_channels: int, num_scalars: int):
        super().__init__()
        self.stem = SequenceConvStem(sequence_channels)
        self.lstm = nn.LSTM(
            input_size=self.stem.out_dim,
        ...
        lstm_out_dim = Config.LSTM_HIDDEN_DIM * (2 if Config.LSTM_BIDIRECTIONAL else 1)
        self.pool = AttentionPooling(lstm_out_dim)
        self.sequence_projector = nn.Sequential(
            nn.Linear(lstm_out_dim, Config.SEQUENCE_PROJECTOR_DIM),
            nn.GELU(),
    ...
    def forward(self, sequences: torch.Tensor, scalars: torch.Tensor, return_aux: bool = False):
        x = self.stem(sequences)
        x = x.transpose(1, 2)
        x, _ = self.lstm(x)
        seq_features = self.sequence_projector(self.pool(x))
...
def build_model(sequence_channels: int, num_scalars: int) -> nn.Module:
    return LSTMSequenceSurrogate(sequence_channels, num_scalars)


if __name__ == "__main__":
    print(f"Running LSTM v1 on device: {Config.DEVICE}")
