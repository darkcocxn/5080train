# -*- coding: utf-8 -*-
"""
2D-CNN 多模态测试脚本（三阶段数据集，小波图版，3 到 7 层）— 进一步优化版
用于评估 `多模态-2DCNN-三阶段数据集-3到7层-强化标量融合-进一步优化.py` 训练得到的模型。
"""
...
def load_training_metadata() -> dict:
    ...
    if architecture:
        ...
        if "use_cbam" in architecture:
            Config.USE_CBAM = bool(architecture["use_cbam"])
        if "cbam_reduction" in architecture:
            Config.CBAM_REDUCTION = int(architecture["cbam_reduction"])
