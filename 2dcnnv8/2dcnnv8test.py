# -*- coding: utf-8 -*-
"""
2D-CNN 多模态测试脚本 v8（三阶段新数据集，小波图版，3 到 7 层）
用于评估 `2dcnnv8.py` 训练得到的模型。
"""
...
def load_training_metadata() -> dict:
    ...
    if metadata.get("wavelet_image_dir"):
        Config.WAVELET_IMAGE_DIR = resolve_portable_path(
            metadata["wavelet_image_dir"],
            default_path=Config.WAVELET_IMAGE_DIR,
        )
    if "force_wavelet_image_dir" in metadata:
        Config.FORCE_WAVELET_IMAGE_DIR = bool(metadata["force_wavelet_image_dir"])
    if metadata.get("wave_metadata_db_path"):
        Config.WAVE_METADATA_DB_PATH = resolve_portable_path(
            metadata["wave_metadata_db_path"],
            default_path=Config.WAVE_METADATA_DB_PATH,
        )
    if metadata.get("wave_metadata_table"):
        Config.WAVE_METADATA_TABLE = str(metadata["wave_metadata_table"])
    if metadata.get("wave_metadata_id_col"):
        Config.WAVE_METADATA_ID_COL = str(metadata["wave_metadata_id_col"])
    if metadata.get("wave_metadata_scalogram_col"):
        Config.WAVE_METADATA_SCALOGRAM_COL = str(metadata["wave_metadata_scalogram_col"])
    ...
    if Config.TEST_CSV_PATH is None and metadata.get("train_csv_path"):
        train_csv_path = resolve_portable_path(metadata["train_csv_path"])
...
def load_test_data():
    ...
    image_paths = build_image_paths(df[Config.IMAGE_COL].values, df[Config.TXT_COL].values)
