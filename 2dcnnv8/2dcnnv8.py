# -*- coding: utf-8 -*-
"""
2D-CNN 多模态训练脚本 v8（三阶段新数据集，小波图版，3 到 7 层）
图像分支: 使用小波图 PNG
...
DEFAULT_DATASET_DIR_NAMES = ("newdata", "CSV-dataset", "数据集")
...
def _config_path_from_env(env_name: str, default_path: Path) -> Path:
...
def resolve_portable_path(path_value: str | Path, default_path: Path | None = None) -> Path:
    text = str(path_value).strip()
    if not text:
        if default_path is None:
            raise ValueError("路径值为空，且未提供默认路径")
        return default_path
...
class Config:
    ...
    NEW_DATA_DIR = _config_path_from_env("SURMOD_NEW_DATA_DIR", PROJECT_ROOT / "newdata")
    NEW_DATABASE_DIR = NEW_DATA_DIR / "Newdatabase"
    WAVE_METADATA_DB_PATH = _config_path_from_env(
        "SURMOD_WAVE_METADATA_DB",
        NEW_DATABASE_DIR / "rare_waves_metadata.db",
    )
    WAVE_METADATA_TABLE = "wave_metadata"
    WAVE_METADATA_ID_COL = "wave_id"
    WAVE_METADATA_SCALOGRAM_COL = "scalogram_path"
    CSV_DIR_CANDIDATES = (
        PROJECT_ROOT / DEFAULT_DATASET_DIR_NAMES[0],
        PROJECT_ROOT / DEFAULT_DATASET_DIR_NAMES[1],
        PROJECT_ROOT / DEFAULT_DATASET_DIR_NAMES[2],
    )
    ...
    WAVELET_IMAGE_DIR = _config_path_from_env(
        "SURMOD_WAVELET_IMAGE_DIR",
        _first_existing_path(
            _latest_existing_dir(NEW_DATABASE_DIR / "Scalogram", "rare_waves-*"),
    ...
    MODEL_ROOT_DIR = PROJECT_ROOT / "output" / "2dcnnv8"
...
_WAVE_SCALOGRAM_PATH_CACHE: dict[int, str] | None = None


def is_missing_path_value(value) -> bool:
    if value is None:
        return True
...
def parse_wave_id_from_reference(value) -> int | None:
...
def resolve_relative_data_path(path_value: str | Path) -> Path:
    path = resolve_portable_path(path_value)
    if path.exists():
        return path
    ...
    candidates = [
        Config.NEW_DATA_DIR / path,
        Config.PROJECT_ROOT / path,
        Config.WAVE_METADATA_DB_PATH.parent / path,
        Config.WAVELET_IMAGE_DIR / path.name,
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
...
def load_wave_scalogram_path_map() -> dict[int, str]:
    global _WAVE_SCALOGRAM_PATH_CACHE
    if _WAVE_SCALOGRAM_PATH_CACHE is not None:
        return _WAVE_SCALOGRAM_PATH_CACHE

    db_path = Path(Config.WAVE_METADATA_DB_PATH)
    if not db_path.exists():
        raise FileNotFoundError(f"未找到地震波-小波图映射数据库: {db_path}")

    query = (
        f'SELECT "{Config.WAVE_METADATA_ID_COL}", "{Config.WAVE_METADATA_SCALOGRAM_COL}" '
        f'FROM "{Config.WAVE_METADATA_TABLE}"'
    )
    mapping: dict[int, str] = {}
    with sqlite3.connect(db_path) as connection:
        for wave_id, scalogram_path in connection.execute(query):
            if scalogram_path is None:
                continue
            mapping[int(wave_id)] = str(resolve_relative_data_path(scalogram_path))

    if not mapping:
        raise ValueError(f"映射数据库中未读取到小波图路径: {db_path}")

    _WAVE_SCALOGRAM_PATH_CACHE = mapping
    return mapping


def resolve_wavelet_image_path(raw_value, wave_reference=None) -> str:
    if not is_missing_path_value(raw_value):
    ...
    wave_id = parse_wave_id_from_reference(wave_reference)
    if wave_id is None:
        raise ValueError(f"无法从地震波引用解析 wave_id: {wave_reference}")

    scalogram_map = load_wave_scalogram_path_map()
    if wave_id not in scalogram_map:
        raise KeyError(f"映射数据库缺少 wave_id={wave_id} 的小波图路径")
    return scalogram_map[wave_id]


def build_image_paths(values, wave_references=None):
    image_paths: list[str] = []
    missing_paths: list[str] = []

    if wave_references is None:
        wave_references = [None] * len(values)
    elif len(values) != len(wave_references):
        raise ValueError(f"图片路径列与地震波引用列长度不一致: {len(values)} vs {len(wave_references)}")

    for value, wave_reference in zip(values, wave_references):
        image_path = Path(resolve_wavelet_image_path(value, wave_reference=wave_reference))
...
def prepare_data():
    ...
    print(f">>> Wave metadata DB:       {Config.WAVE_METADATA_DB_PATH}")
    ...
    train_paths = build_image_paths(train_df[Config.IMAGE_COL].values, train_df[Config.TXT_COL].values)
    val_paths = build_image_paths(val_df[Config.IMAGE_COL].values, val_df[Config.TXT_COL].values)
    ...
    metadata = {
        ...
        "train_csv_path": metadata_path(train_csv_path),
        "val_csv_path": metadata_path(val_csv_path),
        "dataset_search_dirs": [metadata_path(path) for path in get_existing_dataset_dirs()],
        "model_dir": metadata_path(model_dir),
        ...
        "wavelet_image_dir": metadata_path(Config.WAVELET_IMAGE_DIR),
        "force_wavelet_image_dir": bool(Config.FORCE_WAVELET_IMAGE_DIR),
        "wave_metadata_db_path": metadata_path(Config.WAVE_METADATA_DB_PATH),
        "wave_metadata_table": Config.WAVE_METADATA_TABLE,
        "wave_metadata_id_col": Config.WAVE_METADATA_ID_COL,
        "wave_metadata_scalogram_col": Config.WAVE_METADATA_SCALOGRAM_COL,
    ...
    metadata.update(
        {
            ...
            "architecture_revision": "regularized_fusion_tail_prob_gate_v2",
            "best_weights_name": "best_2dcnn_model.pth",
