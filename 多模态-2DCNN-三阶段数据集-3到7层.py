...
class Config:
    ...
    MODEL_SELECTION_SMOOTHING_WINDOW = 3
...
def train():
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        smoothing_window = max(1, int(Config.MODEL_SELECTION_SMOOTHING_WINDOW))
        recent_scores = previous_scores[-(smoothing_window - 1) :] if smoothing_window > 1 else []
        selection_score = float(np.mean([*recent_scores, focus_metrics["focus_score"]]))
