...
```text
使用结构参数、地震动特征/地震动时程和阻尼器布置特征，预测最大层间位移角。
```
...
这组模型面向同一个回归问题。传统 ML 模型默认使用结构参数、地震动派生特征和阻尼器布置特征；MLP 可以先使用同一组表格特征，再扩展为使用地震动时程输入的神经网络版本。这样可以回答两个论文问题：

1. 在结构参数、地震动特征和阻尼器布置特征给定时，传统机器学习和 MLP 对最大层间位移角的预测精度如何？
2. 使用地震动时程作为输入时，相比只使用人工地震动特征是否带来额外增益？
...
```text
输入：结构参数 + 地震动特征/地震动时程 + 阻尼器布置特征
输出：最大层间位移角 max_drift_ratio_raw
```
...
| txt_path | 原始地震动时程文件路径；路径本身不应作为数值特征，只有在时程输入版本中用于读取序列 |
...
建议所有表格模型使用同一套特征，保证公平对比。地震动时程版本不受本节表格特征限制，可将 `txt_path` 指向的时程序列作为 MLP 或其他深度模型输入；传统 ML 基线仍使用地震动派生特征作为时程信息的表格化表征。
...
建议复用当前训练元数据中的 `scalar_feature_names`，共 63 个特征。这样传统 ML 与表格 MLP 的输入完全对齐，便于公平比较。
...
```text
在结构参数、地震动特征和阻尼器布置特征给定时，普通神经网络能达到什么预测水平？
```

它是传统 ML 模型的重要神经网络对比基线。后续如果加入地震动时程输入，可以在表格 MLP 的基础上扩展一个时程分支，用来比较“人工地震动特征”和“原始地震动时程”对最大层间位移角预测的影响。
...
```text
与传统 ML 使用同一组表格输入，公平可比
可以扩展地震动时程分支，验证时程输入是否带来额外收益
训练速度快
容易加入加权 loss、尾部低估惩罚等训练设计
```
...
|---|---|---|---|---|
| B1 | Random Forest | 结构参数 + 地震动特征 + 阻尼器布置特征 | No | y * 1000 |
| B2 | XGBoost | 结构参数 + 地震动特征 + 阻尼器布置特征 | No | y * 1000 |
| B3 | LightGBM | 结构参数 + 地震动特征 + 阻尼器布置特征 | No | y * 1000 |
| B4 | CatBoost | 结构参数 + 地震动特征 + 阻尼器布置特征 | No | y * 1000 |
| B5 | MLP | 结构参数 + 地震动特征 + 阻尼器布置特征 | No | y * 1000 |
| B6 | MLP-TimeHistory（可选） | 结构参数 + 地震动时程 + 阻尼器布置特征 | No | y * 1000 |

第二阶段只对表现最好的 2-3 个模型跑增强版本：

| 实验编号 | 模型 | 输入 | 样本权重 | 目标变换 |
|---|---|---|---|---|
| T1 | Best Tree Model | 结构参数 + 地震动特征 + 阻尼器布置特征 | Tail-weighted | y * 1000 |
| T2 | Best Tree Model | 结构参数 + 地震动特征 + 阻尼器布置特征 | No | log1p(y * 1000) |
| T3 | Best Tree Model | 结构参数 + 地震动特征 + 阻尼器布置特征 | Tail-weighted | log1p(y * 1000) |
| T4 | MLP | 结构参数 + 地震动特征 + 阻尼器布置特征 | Tail-weighted | y * 1000 |
| T5 | MLP-TimeHistory（可选） | 结构参数 + 地震动时程 + 阻尼器布置特征 | Tail-weighted | y * 1000 |
...
| MLP-TimeHistory（可选） | 待训练 | 待训练 | 待训练 | 待训练 | 待训练 | 待训练 | 待训练 |
...
| 8 | MLP-TimeHistory（可选） | 验证地震动时程输入是否优于人工地震动特征 |

## 14. 预期结果与论文解释方式

可能出现三种情况。

第一种情况：传统 ML 与表格 MLP 表现接近。

```text
说明结构参数、地震动派生特征和阻尼器布置特征已经提供了主要预测信息，传统表格模型足以作为强基线。
```

第二种情况：表格 MLP 明显优于传统 ML。

```text
说明最大层间位移角预测中存在传统树模型不易捕捉的连续非线性组合，神经网络对结构参数、地震动特征和阻尼器布置特征的联合表征更有效。
```

第三种情况：MLP-TimeHistory 明显优于表格特征模型。

```text
说明原始地震动时程保留了人工地震动特征未完全覆盖的时序形态信息，论文可以强调地震动时程输入对最大层间位移角预测的增益。
```
...
CatBoost
MLP
MLP-TimeHistory（如果论文需要讨论地震动时程输入）
```
