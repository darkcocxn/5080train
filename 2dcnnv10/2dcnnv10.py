...
BASE_MODULE_NAME = "surmod_v8_train_base_for_v10"
...
def configure_v10(base: ModuleType) -> None:
    cfg = base.Config

    cfg.MODEL_ROOT_DIR = PROJECT_ROOT / "output" / "2dcnnv10"
    ...
    cfg.NUM_EPOCHS = 160
    cfg.SCHEDULER_PATIENCE = 5
    cfg.EARLY_STOPPING_PATIENCE = 42
    cfg.MIN_LR = 3e-7

    cfg.LDS_DENSITY_BIN_COUNT = 48
    cfg.LDS_DENSITY_SMOOTH_KERNEL_SIZE = 9
    cfg.LDS_DENSITY_SMOOTH_SIGMA = 1.6
    cfg.LDS_DENSITY_ALPHA = 0.62

    cfg.LOSS_WEIGHT_MAX = 22.0
    cfg.LOSS_WEIGHT_GE_005 = 1.6
    cfg.LOSS_WEIGHT_GE_010 = 4.5
    cfg.LOSS_WEIGHT_GE_020 = 16.0
    cfg.LOSS_WEIGHT_GE_030 = 22.0

    cfg.TAIL_UNDERPREDICTION_START_EPOCH = 8
    cfg.TAIL_UNDERPREDICTION_RAMP_EPOCHS = 18
    cfg.TAIL_UNDERPREDICTION_WEIGHT = 0.15
    cfg.EXTREME_TAIL_UNDERPREDICTION_WEIGHT = 0.50
    cfg.SEVERE_TAIL_UNDERPREDICTION_THRESHOLD = 0.030
    cfg.SEVERE_TAIL_UNDERPREDICTION_WEIGHT = 0.72
    cfg.TAIL_RELATIVE_UNDER_WEIGHT = 0.03
    cfg.EXTREME_TAIL_RELATIVE_UNDER_WEIGHT = 0.08
    cfg.SEVERE_TAIL_RELATIVE_UNDER_WEIGHT = 0.16
    cfg.TAIL_PINBALL_TAU = 0.60
    cfg.EXTREME_TAIL_PINBALL_TAU = 0.68
    cfg.SEVERE_TAIL_PINBALL_TAU = 0.76
    cfg.TAIL_PINBALL_WEIGHT = 0.012
    cfg.EXTREME_TAIL_PINBALL_WEIGHT = 0.026
    cfg.SEVERE_TAIL_PINBALL_WEIGHT = 0.040
    cfg.TAIL_UNDERPREDICTION_MAX_LOSS = 3.0
    cfg.UNDERPREDICTION_REL_EPS = 5e-4

    cfg.SELECTION_MIN_EPOCH = 30
    cfg.SELECTION_PRE_MIN_EPOCH_OFFSET = 1.0
    ...
    cfg.SELECTION_METRIC = "val_dir_quantile_tail_score_v10"
    ...
    cfg.SAMPLER_MAX_WEIGHT = 20.0
    cfg.SAMPLER_TAIL_BOOST_GE_005 = 1.10
    cfg.SAMPLER_TAIL_BOOST_GE_010 = 1.9
    cfg.SAMPLER_TAIL_BOOST_GE_020 = 5.5
    cfg.SAMPLER_TAIL_BOOST_GE_030 = 8.5
    cfg.SAMPLER_NUM_SAMPLES_MULTIPLIER = 1.18
    cfg.SAMPLER_HIGH_FLOOR_BOOST = 0.28
    cfg.SAMPLER_LOW_STIFFNESS_BOOST = 0.22
    cfg.SAMPLER_LOW_STRENGTH_BOOST = 0.14
    cfg.SAMPLER_HIGH_FLOOR_TAIL_INTERACTION = 0.22

    cfg.TAIL_CLASSIFICATION_THRESHOLDS = [0.010, 0.020, 0.030]
    cfg.TAIL_CLASSIFICATION_LOSS_WEIGHTS = [0.018, 0.055, 0.145]
    ...
    cfg.ARCHITECTURE_REVISION_V10 = "dir_lds_quantile_tail_selection_v10"


def install_v10_patches(base: ModuleType) -> None:
    cfg = base.Config
    ...
    def build_label_density_weights_v10(labels_raw: np.ndarray) -> np.ndarray:
        labels_raw = np.asarray(labels_raw, dtype=np.float64)
        if labels_raw.size == 0 or not cfg.USE_DENSITY_WEIGHTED_LOSS:
            return np.ones(labels_raw.shape, dtype=np.float32)

        label_space = np.log1p(np.maximum(labels_raw, 0.0) * cfg.LABEL_SCALE)
        unique_count = int(np.unique(label_space).size)
        bin_count = max(2, min(int(cfg.LDS_DENSITY_BIN_COUNT), unique_count))
        counts, bin_edges = np.histogram(label_space, bins=bin_count)
        counts = counts.astype(np.float64)

        kernel = base._gaussian_kernel(
            int(cfg.LDS_DENSITY_SMOOTH_KERNEL_SIZE),
            float(cfg.LDS_DENSITY_SMOOTH_SIGMA),
        )
        pad_width = int(len(kernel) // 2)
        padded_counts = np.pad(counts, pad_width=pad_width, mode="edge")
        smoothed_counts = np.convolve(padded_counts, kernel, mode="valid")
        ...
        weights = (median_density / smoothed_counts[bin_indices]) ** float(cfg.LDS_DENSITY_ALPHA)
        weights = weights / max(float(np.mean(weights)), 1e-8)
        weights = np.clip(weights, cfg.LOSS_WEIGHT_MIN, cfg.LOSS_WEIGHT_MAX)
        return weights.astype(np.float32)

    def build_train_sampler_v10(train_df: pd.DataFrame):
    ...
    def apply_tail_loss_multipliers_v10(labels_scaled: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
    ...
    def calculate_tail_underprediction_loss_v10(
        preds: torch.Tensor,
        ...
        def pinball_loss(error: torch.Tensor, tau: float) -> torch.Tensor:
            tau_value = float(tau)
            return torch.maximum(tau_value * error, (tau_value - 1.0) * error)
        ...
        def penalty_for_mask(
            ...
            if not torch.any(mask):
                return preds.new_zeros(())
            signed_error_scaled = labels_scaled[mask] - preds[mask]
            ...
            quantile_loss = torch.mean(pinball_loss(signed_error_scaled, tau))
            return absolute_weight * abs_loss + relative_weight * rel_loss + pinball_weight * quantile_loss
        ...
        total_loss = total_loss + penalty_for_mask(
            ...
            cfg.TAIL_PINBALL_TAU,
            cfg.TAIL_PINBALL_WEIGHT,
        )
        total_loss = total_loss + penalty_for_mask(
            ...
            cfg.EXTREME_TAIL_PINBALL_TAU,
            cfg.EXTREME_TAIL_PINBALL_WEIGHT,
        )
        total_loss = total_loss + penalty_for_mask(
            ...
            cfg.SEVERE_TAIL_PINBALL_TAU,
            cfg.SEVERE_TAIL_PINBALL_WEIGHT,
        )
        return torch.clamp(ramp_factor * total_loss, max=cfg.TAIL_UNDERPREDICTION_MAX_LOSS)

    def calculate_validation_focus_metrics_v10(
    ...
    selection_state = {"epoch": 0}

    def calculate_selection_score_v10(metrics: dict[str, float], focus_metrics: dict[str, float]) -> float:
        selection_state["epoch"] += 1
        full_score = float(
            metrics["mae"]
        ...
        if selection_state["epoch"] < int(cfg.SELECTION_MIN_EPOCH):
            warmup_score = float(
                metrics["mae"]
                + 0.015 * focus_metrics["focus_score"]
                + 0.04 * focus_metrics["mid_tail_mae"]
                + 0.06 * focus_metrics["tail_mae"]
            )
            return float(cfg.SELECTION_PRE_MIN_EPOCH_OFFSET + warmup_score)
        return full_score

    base.build_label_density_weights = build_label_density_weights_v10
    base.build_train_sampler = build_train_sampler_v10
    base.apply_tail_loss_multipliers = apply_tail_loss_multipliers_v10
    base.calculate_tail_underprediction_loss = calculate_tail_underprediction_loss_v10
    base.calculate_validation_focus_metrics = calculate_validation_focus_metrics_v10
    base.calculate_selection_score = calculate_selection_score_v10
...
def postprocess_training_metadata(base: ModuleType) -> None:
    ...
    min_epoch = int(getattr(base.Config, "SELECTION_MIN_EPOCH", 1))
    ...
    if candidates:
        ...
        metadata["best_weights_metric"] = "v10_dir_quantile_candidate_score"

    metadata["v10_overrides"] = {
        "architecture_revision": base.Config.ARCHITECTURE_REVISION_V10,
        "selection_min_epoch": int(base.Config.SELECTION_MIN_EPOCH),
        "lds_density_bin_count": int(base.Config.LDS_DENSITY_BIN_COUNT),
        "lds_density_smooth_kernel_size": int(base.Config.LDS_DENSITY_SMOOTH_KERNEL_SIZE),
        "lds_density_smooth_sigma": float(base.Config.LDS_DENSITY_SMOOTH_SIGMA),
        "lds_density_alpha": float(base.Config.LDS_DENSITY_ALPHA),
        ...
        "tail_pinball_tau": float(base.Config.TAIL_PINBALL_TAU),
        "extreme_tail_pinball_tau": float(base.Config.EXTREME_TAIL_PINBALL_TAU),
        "severe_tail_pinball_tau": float(base.Config.SEVERE_TAIL_PINBALL_TAU),
    }
    metadata["v10_default_checkpoint_selection"] = {
        "metric": "v10_dir_quantile_candidate_score",
        "min_epoch": min_epoch,
        "chosen": chosen,
    ...
    if chosen is not None:
        print(
            ">>> V10 default evaluation weights: "
