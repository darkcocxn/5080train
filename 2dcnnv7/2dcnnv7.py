...
class Config:
    ...
    MODEL_ROOT_DIR = PROJECT_ROOT / "output" / "2dcnnv7"
    ...
    TAIL_UNDERPREDICTION_START_EPOCH = 7
    TAIL_UNDERPREDICTION_RAMP_EPOCHS = 10
    TAIL_UNDERPREDICTION_THRESHOLD = 0.010
    TAIL_UNDERPREDICTION_WEIGHT = 0.16
    EXTREME_TAIL_UNDERPREDICTION_THRESHOLD = 0.020
    EXTREME_TAIL_UNDERPREDICTION_WEIGHT = 0.46
    TAIL_UNDERPREDICTION_MAX_LOSS = 2.0
    ...
    VAL_LOW_DRIFT_THRESHOLD = 0.001
    VAL_LOW_DRIFT_RELATIVE_EPS = 0.0005
    VAL_FOCUS_LOW_MAE_WEIGHT = 0.06
    VAL_FOCUS_LOW_NORM_MAE_WEIGHT = 0.04
    ...
    SELECTION_RMSE_WEIGHT = 0.08
    SELECTION_FOCUS_WEIGHT = 0.018
    SELECTION_LOW_DRIFT_MAE_WEIGHT = 0.025
    SELECTION_LOW_DRIFT_NORM_WEIGHT = 0.020
    ...
    SCALAR_EMBED_DIM = 160
    SCALAR_INPUT_DROPOUT = 0.08
    SCALAR_RES_BLOCKS = 3
    SCALAR_RES_HIDDEN_MULT = 2
    SCALAR_RES_DROPOUT = 0.17
    SCALAR_RESIDUAL_SCALE_INIT = 0.10
    MLP_HIDDEN_LAYERS = [80, SCALAR_EMBED_DIM]
    SCALAR_NORM = "layer"
    MLP_DROPOUT = 0.1

    FUSION_MODE = "gated_bilinear"
    FUSION_BILINEAR_DIM = 64
    FUSION_OUTPUT_DIM = 320
    FUSION_DROPOUT = 0.17
    FUSION_INTERACTION_SCALE_INIT = 0.35

    HEAD_HIDDEN_DIMS = [320, 96]
    HEAD_DROPOUT = 0.23
    USE_TAIL_CORRECTION_HEAD = True
    USE_TAIL_CORRECTION_GATE = True
    TAIL_CORRECTION_HIDDEN_DIM = 80
    TAIL_CORRECTION_DROPOUT = 0.08
    ...
    TAIL_CLASSIFICATION_HIDDEN_DIM = 80
    TAIL_CLASSIFICATION_DROPOUT = 0.08
...
def save_torch_checkpoint(state_dict: dict, path: Path, retries: int = 5, sleep_seconds: float = 0.75) -> None:
    ...
    raise RuntimeError(f"Failed to save checkpoint to {path} after {retries} attempts") from last_error
...
def calculate_low_drift_accuracy_loss(
    preds: torch.Tensor,
    labels_scaled: torch.Tensor,
    epoch_num: int | None = None,
) -> torch.Tensor:
    if not Config.USE_LOW_DRIFT_ACCURACY_LOSS:
        return preds.new_zeros(())
    if epoch_num is not None and epoch_num < Config.LOW_DRIFT_ACCURACY_START_EPOCH:
        return preds.new_zeros(())

    labels_raw = labels_scaled / float(Config.LABEL_SCALE) if Config.SCALE_TARGET else labels_scaled
    preds_raw = preds / float(Config.LABEL_SCALE) if Config.SCALE_TARGET else preds
    low_mask = labels_raw < Config.LOW_DRIFT_THRESHOLD
    if not torch.any(low_mask):
        return preds.new_zeros(())

    if epoch_num is None:
        ramp_factor = 1.0
    else:
        ramp_epochs = max(int(Config.LOW_DRIFT_ACCURACY_RAMP_EPOCHS), 1)
        ramp_factor = min(1.0, max(0.0, (epoch_num - Config.LOW_DRIFT_ACCURACY_START_EPOCH + 1) / ramp_epochs))

    denom = labels_raw[low_mask].abs() + float(Config.LOW_DRIFT_RELATIVE_EPS)
    normalized_error = (preds_raw[low_mask] - labels_raw[low_mask]) / denom
    low_loss = nn.functional.smooth_l1_loss(
        normalized_error,
        torch.zeros_like(normalized_error),
        beta=0.35,
        reduction="mean",
    )
    low_loss = ramp_factor * Config.LOW_DRIFT_ACCURACY_WEIGHT * low_loss
    return torch.clamp(low_loss, max=Config.LOW_DRIFT_ACCURACY_MAX_LOSS)
...
def calculate_validation_focus_metrics(y_true_raw: np.ndarray, y_pred_raw: np.ndarray) -> dict[str, float]:
    ...
    low_drift_mask = y_true_raw < Config.VAL_LOW_DRIFT_THRESHOLD
    ...
    if np.any(low_drift_mask):
        low_errors = errors[low_drift_mask]
        low_abs_errors = np.abs(low_errors)
        low_drift_mae = float(np.mean(low_abs_errors))
        low_drift_bias = float(np.mean(low_errors))
        low_drift_under_mae = float(np.mean(under_errors[low_drift_mask]))
        low_drift_over_mae = float(np.mean(np.maximum(y_pred_raw[low_drift_mask] - y_true_raw[low_drift_mask], 0.0)))
        low_drift_norm_mae = float(
            np.mean(low_abs_errors / (np.abs(y_true_raw[low_drift_mask]) + Config.VAL_LOW_DRIFT_RELATIVE_EPS))
        )
        low_drift_mape = float(np.mean(low_abs_errors / np.maximum(np.abs(y_true_raw[low_drift_mask]), 1e-12)) * 100.0)
    else:
        low_drift_mae = float(np.mean(np.abs(errors)))
        low_drift_bias = float(np.mean(errors))
        low_drift_under_mae = float(np.mean(under_errors))
        low_drift_over_mae = float(np.mean(np.maximum(errors, 0.0)))
        low_drift_norm_mae = float(np.mean(np.abs(errors) / (np.abs(y_true_raw) + Config.VAL_LOW_DRIFT_RELATIVE_EPS)))
        low_drift_mape = float(np.mean(np.abs(errors) / np.maximum(np.abs(y_true_raw), 1e-12)) * 100.0)
    ...
    focus_score = (
        ...
        + Config.VAL_FOCUS_LOW_MAE_WEIGHT * low_drift_mae
        + Config.VAL_FOCUS_LOW_NORM_MAE_WEIGHT * (low_drift_norm_mae * Config.VAL_LOW_DRIFT_RELATIVE_EPS)
    ...
    return {
        ...
        "low_drift_mae": float(low_drift_mae),
        "low_drift_bias": float(low_drift_bias),
        "low_drift_under_mae": float(low_drift_under_mae),
        "low_drift_over_mae": float(low_drift_over_mae),
        "low_drift_norm_mae": float(low_drift_norm_mae),
        "low_drift_mape": float(low_drift_mape),
        "low_drift_count": int(np.sum(low_drift_mask)),
...
def calculate_selection_score(metrics: dict[str, float], focus_metrics: dict[str, float]) -> float:
    return float(
        metrics["mae"]
        + Config.SELECTION_RMSE_WEIGHT * metrics["rmse"]
        + Config.SELECTION_FOCUS_WEIGHT * focus_metrics["focus_score"]
        + Config.SELECTION_LOW_DRIFT_MAE_WEIGHT * focus_metrics["low_drift_mae"]
        + Config.SELECTION_LOW_DRIFT_NORM_WEIGHT
        * (focus_metrics["low_drift_norm_mae"] * Config.VAL_LOW_DRIFT_RELATIVE_EPS)
...
def train():
    ...
    metadata.update(
        {
            ...
            "architecture_revision": "v7_delayed_low_drift_guarded_selection",
            "best_weights_name": "best_2dcnn_model.pth",
            ...
            "low_drift_accuracy_loss": {
                "enabled": Config.USE_LOW_DRIFT_ACCURACY_LOSS,
                "threshold": Config.LOW_DRIFT_THRESHOLD,
                "relative_eps": Config.LOW_DRIFT_RELATIVE_EPS,
                "weight": Config.LOW_DRIFT_ACCURACY_WEIGHT,
                "start_epoch": Config.LOW_DRIFT_ACCURACY_START_EPOCH,
                "ramp_epochs": Config.LOW_DRIFT_ACCURACY_RAMP_EPOCHS,
                "max_loss": Config.LOW_DRIFT_ACCURACY_MAX_LOSS,
            },
            "validation_selection": {
                "metric": Config.SELECTION_METRIC,
                "mae_component": "val_mae_raw",
                "rmse_component": "val_rmse_raw",
                "rmse_blend_weight": Config.SELECTION_RMSE_WEIGHT,
                "focus_component": "focus_score",
                "focus_blend_weight": Config.SELECTION_FOCUS_WEIGHT,
                "low_drift_component": "low_drift_mae",
                "low_drift_norm_component": "low_drift_norm_mae",
                "low_drift_threshold": Config.VAL_LOW_DRIFT_THRESHOLD,
                "low_drift_relative_eps": Config.VAL_LOW_DRIFT_RELATIVE_EPS,
                "low_drift_mae_weight": Config.SELECTION_LOW_DRIFT_MAE_WEIGHT,
                "low_drift_norm_weight": Config.SELECTION_LOW_DRIFT_NORM_WEIGHT,
                "focus_low_mae_weight": Config.VAL_FOCUS_LOW_MAE_WEIGHT,
                "focus_low_norm_mae_weight": Config.VAL_FOCUS_LOW_NORM_MAE_WEIGHT,
    ...
    best_mae_epoch = 0
    best_rmse_checkpoint = float("inf")
    best_rmse_epoch = 0
    best_r2_checkpoint = -float("inf")
    best_r2_epoch = 0
    best_extreme_under_checkpoint = float("inf")
    best_extreme_under_epoch = 0
    best_low_drift_checkpoint = float("inf")
    best_low_drift_epoch = 0
    best_epoch = 0
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        for imgs, scalars, labels, sample_weights in loop:
            ...
            with build_amp_autocast_context(use_amp):
                ...
                loss = loss + calculate_low_drift_accuracy_loss(preds, labels, epoch_num=epoch + 1)
        ...
        history["val_low_drift_mae_raw"].append(focus_metrics["low_drift_mae"])
        history["val_low_drift_bias_raw"].append(focus_metrics["low_drift_bias"])
        history["val_low_drift_under_mae_raw"].append(focus_metrics["low_drift_under_mae"])
        history["val_low_drift_over_mae_raw"].append(focus_metrics["low_drift_over_mae"])
        history["val_low_drift_norm_mae"].append(focus_metrics["low_drift_norm_mae"])
        history["val_low_drift_mape"].append(focus_metrics["low_drift_mape"])
        history["val_low_drift_count"].append(focus_metrics["low_drift_count"])
        ...
        if Config.SAVE_ALTERNATE_BEST_CHECKPOINTS:
            if focus_metrics["focus_score"] < best_raw_focus_score:
                ...
                save_torch_checkpoint(checkpoint_state, Config.SAVE_DIR / "best_2dcnn_focus_model.pth")
            if metrics["mae"] < best_mae_checkpoint:
                best_mae_checkpoint = metrics["mae"]
                best_mae_epoch = epoch + 1
                save_torch_checkpoint(checkpoint_state, Config.SAVE_DIR / "best_2dcnn_mae_model.pth")
            if metrics["rmse"] < best_rmse_checkpoint:
                best_rmse_checkpoint = metrics["rmse"]
                best_rmse_epoch = epoch + 1
                save_torch_checkpoint(checkpoint_state, Config.SAVE_DIR / "best_2dcnn_rmse_model.pth")
            if metrics["r2"] > best_r2_checkpoint:
                best_r2_checkpoint = metrics["r2"]
                best_r2_epoch = epoch + 1
                save_torch_checkpoint(checkpoint_state, Config.SAVE_DIR / "best_2dcnn_r2_model.pth")
            if focus_metrics["extreme_tail_under_mae"] < best_extreme_under_checkpoint:
                ...
                save_torch_checkpoint(checkpoint_state, Config.SAVE_DIR / "best_2dcnn_extreme_under_model.pth")
            if focus_metrics["low_drift_norm_mae"] < best_low_drift_checkpoint:
                best_low_drift_checkpoint = focus_metrics["low_drift_norm_mae"]
                best_low_drift_epoch = epoch + 1
                save_torch_checkpoint(checkpoint_state, Config.SAVE_DIR / "best_2dcnn_low_drift_model.pth")
        ...
        if improved:
            ...
            best_epoch = epoch + 1
            early_stop_counter = 0
            save_torch_checkpoint(checkpoint_state, Config.SAVE_DIR / "best_2dcnn_model.pth")
            tqdm.write(
                "  >> Best model saved "
                f"(Selection score: {selection_score:.6f}, "
                f"Focus score: {focus_metrics['focus_score']:.6f}, "
                f"MAE: {best_val_mae_raw:.6f}, "
                f"low MAE: {focus_metrics['low_drift_mae']:.6f}, "
                f"low norm: {focus_metrics['low_drift_norm_mae']:.4f}, "
        ...
        tqdm.write(
            ...
            f"Selection={selection_score:.6f} | "
                f"Low MAE={focus_metrics['low_drift_mae']:.6f} | "
                f"Low Norm={focus_metrics['low_drift_norm_mae']:.4f} | "
                f"Low MAPE={focus_metrics['low_drift_mape']:.2f}% | "
    ...
    save_torch_checkpoint(model.state_dict(), final_model_path)
    if ema is not None:
        save_torch_checkpoint(ema.state_dict(), Config.SAVE_DIR / "ema_2dcnn_model.pth")
    ...
    metadata.update(
        {
            ...
            "alternate_best_checkpoints": {
                ...
                "rmse": {
                    "weights_name": "best_2dcnn_rmse_model.pth",
                    "epoch": int(best_rmse_epoch),
                    "rmse": float(best_rmse_checkpoint),
                },
                "r2": {
                    "weights_name": "best_2dcnn_r2_model.pth",
                    "epoch": int(best_r2_epoch),
                    "r2": float(best_r2_checkpoint),
                },
                "extreme_under": {
                ...
                "low_drift": {
                    "weights_name": "best_2dcnn_low_drift_model.pth",
                    "epoch": int(best_low_drift_epoch),
                    "norm_mae": float(best_low_drift_checkpoint),
                },
            },
...
def plot_results(history):
    ...
    if "val_low_drift_mae_raw" in history:
        plt.plot(history["val_low_drift_mae_raw"], label="Low-Drift MAE", color="cyan", alpha=0.8)
