# -*- coding: utf-8 -*-
"""
2D-CNN 多模态训练脚本（三阶段数据集，小波图版，3 到 7 层）— 进一步优化版
基于强化标量融合版本，新增以下优化：
  1. R-Drop 正则化：两次前向传播 + MSE 一致性惩罚，直接对抗过拟合；
  2. CosineAnnealingWarmRestarts：替代 ReduceLROnPlateau，更稳定的学习率调度；
  3. SWA (Stochastic Weight Averaging)：训练后期启用，寻找更平坦的极小值；
  4. Log-Cosh Loss：全光滑损失函数，替代 SmoothL1，梯度行为更好；
  5. C-Mixup：回归专用 mixup，按标签相似度配对样本，改善低图像多样性问题；
  6. CBAM 注意力：通道+空间注意力模块，增强 CNN 特征表达；
  7. 增强图像增广：高斯模糊 + 随机擦除，缓解仅 167 张唯一小波图的瓶颈。
"""
...
class Config:
    ...
    MODEL_ROOT_DIR = SCRIPT_DIR / "model-2dcnn-3stage-rf-3to7-strong-scalar-fusion-v2"
    ...
    # ── 进一步优化：CosineAnnealingWarmRestarts ──
    USE_COSINE_ANNEALING = True
    COSINE_T_0 = 15
    COSINE_T_MULT = 2
    COSINE_ETA_MIN = 1e-6

    # ── 进一步优化：SWA (Stochastic Weight Averaging) ──
    USE_SWA = True
...
def log_cosh_loss(preds: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    diff = preds - targets
    return torch.mean(diff + F.softplus(-2.0 * diff) - 0.6931471805599453)
...
def build_image_transform(train: bool) -> transforms.Compose:
    ...
    if train and Config.USE_GAUSSIAN_BLUR:
        transform_steps.append(
            transforms.RandomApply(
                [transforms.GaussianBlur(kernel_size=list(Config.GAUSSIAN_BLUR_KERNEL), sigma=Config.GAUSSIAN_BLUR_SIGMA)],
    ...
    if train and Config.USE_RANDOM_ERASING:
        transform_steps.append(
            transforms.RandomErasing(p=Config.RANDOM_ERASING_PROB, scale=(0.02, 0.15), ratio=(0.3, 3.3), value=0.5)
        )
...
class ConditionalResidualConvBlock2D(nn.Module):
    def __init__(
        ...
        self.cbam = CBAM(out_channels, reduction=Config.CBAM_REDUCTION) if Config.USE_CBAM else None
...
def cmixup_batch(
    ...
    if batch_size <= 1 or alpha <= 0.0:
        return images, scalars, labels, sample_weights
    ...
    indices = torch.tensor(indices, dtype=torch.long, device=images.device)

    mixed_images = lam * images + (1.0 - lam) * images[indices]
    mixed_scalars = lam * scalars + (1.0 - lam) * scalars[indices]
    mixed_labels = lam * labels + (1.0 - lam) * labels[indices]
    mixed_weights = lam * sample_weights + (1.0 - lam) * sample_weights[indices]
    return mixed_images, mixed_scalars, mixed_labels, mixed_weights
...
def train():
    ...
    metadata.update(
        {
            ...
            "architecture_revision": "v3_rdrop_swa_cmixup_cbam_logcosh",
            "best_weights_name": "best_2dcnn_model.pth",
            ...
            "loss": "LogCoshLoss" if Config.USE_LOG_COSH_LOSS else "SmoothL1Loss",
            ...
            "scheduler": {
                "type": "CosineAnnealingWarmRestarts" if Config.USE_COSINE_ANNEALING else "ReduceLROnPlateau",
            ...
            "optimizations_v2": {
                "r_drop": {"enabled": Config.USE_R_DROP, "alpha": Config.R_DROP_ALPHA},
                "swa": {"enabled": Config.USE_SWA, "start_ratio": Config.SWA_START_EPOCH_RATIO, "lr": Config.SWA_LR},
                "cmixup": {"enabled": Config.USE_CMIXUP, "alpha": Config.CMIXUP_ALPHA, "sigma": Config.CMIXUP_SIGMA},
                "log_cosh_loss": Config.USE_LOG_COSH_LOSS,
                "cbam": {"enabled": Config.USE_CBAM, "reduction": Config.CBAM_REDUCTION},
                "gaussian_blur": {"enabled": Config.USE_GAUSSIAN_BLUR, "prob": Config.GAUSSIAN_BLUR_PROB},
                "random_erasing": {"enabled": Config.USE_RANDOM_ERASING, "prob": Config.RANDOM_ERASING_PROB},
            },
            "sampler_params": {
    ...
    swa_model = None
    swa_scheduler = None
    swa_start_epoch = int(Config.NUM_EPOCHS * Config.SWA_START_EPOCH_RATIO) if Config.USE_SWA else Config.NUM_EPOCHS + 1
    if Config.USE_SWA:
    ...
    in_swa_phase = False
    print(f"    R-Drop: {'ON' if Config.USE_R_DROP else 'OFF'} | "
          f"C-Mixup: {'ON' if Config.USE_CMIXUP else 'OFF'} | "
          f"CBAM: {'ON' if Config.USE_CBAM else 'OFF'} | "
          f"Log-Cosh: {'ON' if Config.USE_LOG_COSH_LOSS else 'OFF'} | "
          f"SWA: {'ON (epoch '+str(swa_start_epoch)+')' if Config.USE_SWA else 'OFF'}")
    start_time = time.time()

    for epoch in range(Config.NUM_EPOCHS):
        apply_lr_warmup(optimizer, epoch)
        in_swa_phase = Config.USE_SWA and epoch >= swa_start_epoch
        model.train()
        train_loss = 0.0
        ...
        for imgs, scalars, labels, sample_weights in loop:
            ...
            # ── C-Mixup ──
            if Config.USE_CMIXUP and not in_swa_phase:
                imgs, scalars, labels, sample_weights = cmixup_batch(
                    imgs, scalars, labels, sample_weights,
                    alpha=Config.CMIXUP_ALPHA,
                    sigma=Config.CMIXUP_SIGMA,
                )
            ...
            with build_amp_autocast_context(use_amp):
                ...
                # ── 主回归损失 ──
                if Config.USE_LOG_COSH_LOSS:
                    loss_weights = apply_tail_loss_multipliers(labels, sample_weights)
                    diff = preds - labels
                    per_sample_logcosh = diff + F.softplus(-2.0 * diff) - 0.6931471805599453
                    loss = (per_sample_logcosh * loss_weights).sum() / loss_weights.sum().clamp_min(1.0)
                else:
                ...
                # ── R-Drop 一致性正则 ──
                if Config.USE_R_DROP and not in_swa_phase:
                    preds2 = model(imgs, scalars)
                    r_drop_loss = F.mse_loss(preds, preds2)
                    loss = loss + Config.R_DROP_ALPHA * r_drop_loss
            ...
            scaler.update()
            if ema is not None and not in_swa_phase:
                ema.update(model)
        ...
        checkpoint_state = ema.state_dict() if (ema is not None and not in_swa_phase) else model.state_dict()
        if ema is not None and not in_swa_phase:
            ema.restore(model)
        ...
        if in_swa_phase and swa_scheduler is not None:
            swa_scheduler.step()
        elif Config.USE_COSINE_ANNEALING:
